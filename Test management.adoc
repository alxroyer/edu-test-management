...
et xref:section-impact-analyses-strategy[xrefstyle=full]
pour des compléments sur la gestion des modifications des exigences.
...
(cf. xref:[xrefstyle=full] pour une discussion
sur les stratégies possibles d'analyses d'impact).
...
[cols="1,1"]
.Testeur v/s développeur, comparaison des profils
|===
| Développeur | Testeur
...
Premièrement,
comme indiqué en xref:section-timing-test-auto[],
...
utilisant une ou plusieurs base de données
(typiquement les applications Web),
se pose une problématique de gestion des données.

En effet, le test est dépendant des données existantes dans la base.
Or ces données peuvent être amenées à vivre dans le temps.

[NOTE]
====
Pour les autres types d'applications,
la problématique est souvent plus simple voire inexistante,
car les données sont peu volumineuses,
et peuvent être constituées programmatiquement.
====

[TIP]
====
Pour pallier les risques de disparition
ou d'altération d'une donnée de référence utilisée dans un test,
on peut mettre en place un mécanisme de recherche dynamique d'une donnée
sur la base de critères requis pour un test donné.
====
...
Les langages communément utilisés
pour développer des tests automatiques
sont des *langages de scripting* :

* Javascript, notamment dans le monde des développements Web,
* Python.

[TIP]
.Rappel utilité vérification de typage
====
Pour mémo, si les langages utilisés pour les tests automatiques
sont des langages de scripting,
a priori non typés,
le fait de mettre en place de la vérification de typage
reste une bonne pratique
(cf. xref:section-type-checking[xrefstyle=full]).
====

Une autre option de langage existe également : *Gherkin*
(cf. https://cucumber.io/docs/gherkin/reference).

La philosophie de ce langage est d'approcher le langage naturel
pour la description de tests fonctionnels
(approche _Behaviour Driven Development - BDD_).

L'intérêt principal de ce langage
est de constituer un langage compréhensible
par les différentes parties prenantes,
qu'elles soient techniques ou non,
telles que les _Product Owners_ notamment.

La traduction en tests automatiques exécutable
se fait ensuite au moyen de _step definitions_
(en terminologie Gherkin, cf. https://cucumber.io/docs/gherkin/step-organization).
...
Le tableau ci-après liste quelques outils
souvent utilisés pour les tests réseaux :

[cols="1,2,5"]
.Librairies et outils réseau
|===
| Outils | URL | Description

| curl | https://curl.se/
a|
Outil en ligne de commandes
permettant de forger des requêts HTTP.

Peut être utilisé accessoirement pour des tests d'API
(cf. xref:section-frameworks-api-testing[xrefstyle=full]).

| iperf | https://iperf.fr/
a|
Permet la mesure de performances en bande passante
sur des réseaux IP.

| Scapy | https://scapy.net/
a|
Librairie Python
permettant la sérialisation / désérialisation
de paquet réseaux : Ethernet, IP, ...

Possibilité de définir des formats spécifiques.

Peut être utilisé pour sérialiser / désérialiser
toute donnée binaire
(fichier, zone mémoire, ...).
|===
...
l'unité testée est une classe ou une fonction.

Comme cette unité testée ne peut être lancée seule,
les tests unitaires sont généralement écrits
dans le même langage que le code testé,
en mode boîte blanche
(cf. xref:section-black-box-white-box[xrefstyle=full]).
...
| PyUnit / unittest | Python | https://docs.python.org/3/library/unittest.html
| PyTest | Python | https://docs.pytest.org/
|===


[#section-frameworks-api-testing]
==== Tests d'API

Le tableau ci-après liste des frameworks
permettant de réaliser des tests d'API :
...
[sidebar]
--
👷🛠️ TP : Test d'API avec SoapUI (TODO)
--
...
Le tableau ci-après liste des frameworks
permettant de réaliser des tests fonctionnels :
...
Le tableau ci-après liste des outils
permettant de réaliser des tests de charge et de performances :

[cols="1,2,5"]
.Outils de tests de charge et performances
|===
| Outils | URL | Description
...
| https://www.opentext.com/products/enterprise-performance-engineering
a|
Anciennement HP LoadRunner.
...
| NeoLoad | https://www.tricentis.com/fr/produits/test-de-performance-neoload
a|
TODO
|===
...
L'exécution de tests auto
donne l'opportunité de mesurer une couverture de code
associée à une campagne de tests.

Cela permet d'identifier les zones de code
par lesquelles le SUT est passé
alors qu'il était testé,
et inversement, les zones par lesquelles il n'est pas passé.

Cette mesure permet d'avoir une estimation
de la profondeur des tests :
les tests sont-ils suffisamment couvrant ou non ?

Dans le cas de logiciels de sécurité,
cette mesure de couverture cherche également
à mettre en évidence l'existence de code mort,
qu'on souhaite éviter.

[WARNING]
.Code défensif, code mort et couverture de code
====
Il arrive régulièrement qu'on écrive
un certain nombre de tests sur la validité des données
en entrée d'une fonction bas niveau.

A priori, les appels à la fonction dans le logiciel
ne sont pas censés passer des données invalides.
Mais sait-on jamais :
c'est le principe du code défensif.

Dans le cadre d'une mesure de couverture de code,
avec des tests fonctionnels,
ce code défensif ne devrait jamais être appelé
(ou alors cela signifie qu'un appel à la fonction
avec des données invalides
est fait quelque part dans le code).

L'existence de code défensif est une bonne chose en soit
pour la sécurité,
mais elle génère des faux positifs en termes de détection de code mort,
et une dégradation des mesures de couverture de code.
====

[NOTE]
====
D'expérience personnelle,
les mesures de couverture de code
sont généralement une demande du management
ou un attendu contractuel.
====

[sidebar]
--
👷🛠️ TP : Couverture de code (TODO)
--


== Management des tests

Cette section regroupe des conseils
sur le pilotage d'une activité de test.


[#section-timing-test-auto]
=== Timing pour la constitution des tests auto

On a souvent pour habitude,
de dérouler les activités de test majoritairement
*après le développement du SUT*.

Cela mène souvent à des détection d'erreurs tardives.
Or, plus la détection d'une erreur est tardive,
plus elle coûte cher à corriger.

[NOTE]
====
Dans le cas des systèmes complexes
(cf. xref:section-subsystems[xrefstyle=full]),
pour les niveaux de systèmes intégrant des sous-systèmes,
on n'a pas trop le choix de la formule :
on devra attendre la livraison des sous-systèmes
avant de démarrer efficacement les activités de test.
====

---

Dans le but de pallier ces coûts de détection tardive des erreurs,
sont nées les approches *_Test Driven Development_ (TDD)*,
notamment pour les développements logiciels.

Le principe des TDD est d'écrire les tests,
et notamment les tests automatiques,
*avant le développement du SUT*.

En pratiquant le TDD quelque peu,
on peut observer le confort que cela apporte aux développeurs :

* ils disposent directement d'un moyen d'exécution de leur travail,
* ils savent identifier clairement l'objectif à atteindre,
  et peuvent se concentrer sur leur développement
  jusqu'à ce que les tests donnent un statut OK.

Dans les faits, les tests nécessitent beaucoup de reprise,
car cela impose au départ de rédiger les tests auto à l'aveugle,
puis du temps doit être consacré au moment de l'intégration avec le SUT.

Par ailleurs, le fait de mener cette activité en deux temps
crée des changements de contextes coûteux
(idem xref:section-non-regression-time[xrefstyle=full]).

---

Après expérimentations,
le meilleur compromis qui s'impose
est de mener l'activité de test
*conjointement avec le développement du SUT*.

En procédant ainsi,
le testeur évite les changements de contexte,
et reste sur son élan du début à la fin
du développement et du test d'une fonction donnée.

Par ailleurs, le développeur et le testeur,
travaillant en binôme,
s'alimentent l'un l'autre :

* le testeur fournit
  un moyen de test et intégration,
  ainsi qu'un objectif à atteindre
  au développeur pour ses travaux en cours,
* le développeur fournit des premières versions de SUT au testeur
  qui peut avancer sans être à l'aveugle.

Enfin, dès qu'une erreur est détectée par le testeur,
elle est immédiatement traitée et intégrée par le développeur,
sans même perdre de temps à créer de rapport d'erreur
(cf. xref:section-issue-management[xrefstyle=full]).

On garde ainsi le confort apporté aux développeurs par les tests,
et on répond à la fois
aux problèmes de détections tardives des erreurs,
et aux problèmes de changements de contexte coûteux pour le testeur.


[#section-impact-analyses-strategy]
=== Stratégie d'analyses d'impact

En cas de modification des exigences
(cf. xref:section-lifecycle-req-change[xrefstyle=full]),
on distingue deux stratégies possibles
pour les analyses d'impact :
une stratégie dite _cycle en V_,
et une stratégie dite _latest_ ou "crête de vague".

---

La stratégie *_cycle en V_*
consiste à considérer des baselines arrêtées des spécifications,
avec des numéros de version
identifiant des publications des documents.

Par conséquent les analyses d'impact s'opèrent
sur l'intégralité des modifications
entre deux versions des documents de spécifications.
...
L'idée est de faire avancer les spécifications,
le code et les tests en même temps
pour chaque ticket d'évolution
(cf. xref:section-issue-management[xrefstyle=full])).

A noter que, pour des raisons de gestion du planning,
cette stratégie n'est possible
que sur sur un développement de petite taille,
...
résume les différences majeures entre ces deux stratégies :

.Stratégies d'analyses d'impact
[cols=">.<1,^.^2,^.^2"]
|===
h|
.>h| Cycle en V
.>h| _latest_ / "crête de vague"

h| Projet type
a| Intégration de sous-systèmes
a| Logiciel simple

h| Temps
a| Long
a| Court, Kanban ou Sprints agiles
...
h| Granularité
a| Document
a| Ticket d'évolution

h| Avantages
a|
Maîtrise de la baseline fonctionnelle
a|
Fuidité

Réduction de l'effort de versioning des exigences
|===


=== Planification de la constitution des tests

Au moment de démarrer le projet,
une question peut se poser :
dans quel ordre constituer les tests ?

En réalité, cela n'est pas dépendant
du mode de développement : cycle en V ou méthodes agiles.

La dépendance vient plutôt de la *stratégie d'intégration* :
on ne pourra tester que ce qui sera livrer au file du temps.
...
On a évoqué en xref:section-requirement-management[xrefstyle=full]
le besoin d'assurer une complétude des tests
par rapport aux exigences
et à tous les aspects de chaque exigence.

En pratique, l'exhaustivité parfaite des tests n'existe pas :
c'est un exercice infini
dans la mesure où on peut toujours rentrer dans des détails
de plus en plus fins.

Cela implique des coûts non négligeables.
Or les projets ne fonctionnent pas à budget illimité.


==== Stratégie de test par les risques

C'est pourquoi il est utile de considérer la stratégie
en regard des risques de non-qualité du SUT.

On rentre alors dans une démarche d'investissement et de ROI :
quel effort de test mettre,
pour couvrir quels risques de non-qualité ?


==== Tests unitaires v/s tests fonctionnels

En matière de gestion de risque,
une question peut se poser sur l'effort à mettre
entre tests unitaires d'un part,
et tests fonctionnels d'autre part.

Les tests unitaires permettent d'atteindre
des couvertures de code plus importantes
(cf. xref:section-code-coverage[xrefstyle=full]).

Toutefois, les tests unitaires ne couvrent pas l'intégration
des différentes parties du logiciel,
et donc ne garantissent pas le fonctionnel final attendu.
Aussi on pourra privilégier les tests fonctionnels
pour ces raisons.

Toutefois encore, quelques actions de tests unitaires ciblées
pourront réduire les risques de difficultés en intégration.


==== Tests manuels v/s tests auto

Toujours en matière de gestion de risque,
une question peut se poser sur l'effort à mettre
entre tests automatiques d'une part,
et tests manuels d'autre part.

On a évoqué en xref:section-test-auto-mandatory[xrefstyle=full]
l'obligation d'automatiser les tests pour l'agilité.

Pour autant, l'humain garde une plus-value
sur certaines activités de test,
notamment :

* les activités d'intégration,
  car beaucoup de problèmes peuvent survenir,
  et il convient de savoir réagir,
* les activités de validation du besoin utilisateur,
  pour les capacités d'empathie requises dans cet exercice.
...
Dans le cas de systèmes complexes
(cf. xref:section-subsystems[xrefstyle=full]),
une autre marge d'ajustement de l'effort de test peut être envisagé :
les différentes couches de tests à chaque niveau du système
peuvent présenter un certain nombre de redondances.

On pourra donc, par exemple, identifier les exigences qui redescendent en 1 pour 1
entre un système et un des ses sous-systèmes seulement.
Dans cette situation, on pourra décider de faire l'effort de test
sur le sous-système ou sur le système intégré.
